#!/usr/bin/env python3
"""
Google Sheets Reader - Secure authenticated access to time tracking spreadsheet
"""
...
class SheetsReader:
    ...
    def get_wip_entries(self, worksheet_name: str = "Sheet1") -> List[Dict[str, Any]]:
        ...
        try:
            ...
            paid_col = self._find_column_index(headers, ['paid'])
    ...
    def update_wip_to_billed(self, wip_entries: List[Dict[str, Any]], worksheet_name: str = "Sheet1") -> bool:
        """
        Update WIP entries to 'Paid' status after invoice generation.
        ...
        try:
            ...
            # Get headers to find the 'Paid' column
            headers = worksheet.row_values(1)
            paid_col_index = self._find_column_index(headers, ['paid']) + 1  # gspread uses 1-based indexing
            
            if paid_col_index > len(headers):
                print("⚠️  Warning: 'Paid' column not found, cannot update status")
                return False
            
            # Update each WIP entry to 'Paid'
            updates = []
            for entry in wip_entries:
                row_number = entry.get("row_number")
                if row_number:
                    # Create update for this cell
                    cell_address = f"{chr(64 + paid_col_index)}{row_number}"  # Convert to A1 notation
                    updates.append({
                        'range': cell_address,
                        'values': [['Paid']]
                    })
            ...
            if updates:
                ...
                print(f"✅ Updated {len(updates)} entries from WIP to Paid")
                return True
